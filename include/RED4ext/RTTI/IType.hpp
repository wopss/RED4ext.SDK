...
struct IType
{
    ...
    virtual void Destruct(void* aMemory) const = 0;  // 40
    virtual const bool IsEqual(const void* aLhs, const void* aRhs,
                               uint32_t a3 = 0) = 0; // 48 - Not const because CClass aquire some mutex when this is
                                                     // called and a flag is modified.
    virtual void Assign(void* aLhs, const void* aRhs) const = 0;                          // 50
    virtual void Move(void* aLhs, void* aRhs) const;                                      // 58
    virtual bool Unserialize(BaseStream* aStream, void* aInstance, int64_t a3) const = 0; // 60
    virtual bool ToString(const void* aInstance, CString& aOut) const;                    // 68
    virtual bool FromString(void* aInstance, const CString& aString) const;               // 70
    virtual bool sub_78();                                                                // 78
    virtual bool sub_80(int64_t a1, void* aInstance);                                     // 80
    virtual bool sub_88(int64_t a1, void* aInstance);                                     // 88
    virtual bool sub_90(int64_t a1, void* aInstance, CString& a3, int64_t a4);            // 90
    virtual bool sub_98(int64_t a1, void* aInstance, CString& a3, int64_t a4, bool a5);   // 98
...
#pragma region deprecated
    ...
    inline void Init(void* aMemory) const
    {
        Construct(aMemory);
    }

    [[deprecated("Use 'Destruct()' instead.")]]
    inline void Destroy(void* aMemory) const
    {
        Destruct(aMemory);
    }
#pragma endregion
